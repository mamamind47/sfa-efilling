...
const EditProjectPage = () => {
  ...
  const fetchProjectData = async () => {
    try {
      ...
      const startDate = new Date(project.start_date).toISOString().slice(0, 16);
      const endDate = new Date(project.end_date).toISOString().slice(0, 16);
      ...
      setFormData({
        ...
        hours_per_person: project.hours_per_person,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If project was rejected, resubmit it
      if (projectStatus === 'rejected') {
        await apiClient.post(`/projects/${projectId}/resubmit`);
        toast.success("บันทึกการเปลี่ยนแปลงและส่งโครงการใหม่เรียบร้อย");
      } else {
      ...
      toast.error(error.response?.data?.error || "เกิดข้อผิดพลาดในการแก้ไขโครงการ");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="card bg-white shadow-md mb-4">
          <div className="card-body p-4">
            <h1 className="text-lg font-bold text-gray-800">แก้ไขกิจกรรม</h1>
            <p className="text-xs text-gray-600 mt-1">โปรดตรวจสอบข้อมูลกิจกรรมและแก้ไขให้ถูกต้อง</p>
          </div>
        </div>
              ...
              <div className="flex justify-between pt-4 border-t mt-4">
                ...
                >
                  {submitting ? (
                    ...
                    "บันทึกการเปลี่ยนแปลงและขอนุมัติอีกครั้ง"
                  ) : (
                    "บันทึกการแก้ไข"
                  )}
                </button>
